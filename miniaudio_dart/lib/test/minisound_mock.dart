...
class MiniaudioDartMock extends MiniaudioDartPlatform {
  @override
...
class EngineMock implements PlatformEngine {
  ...
  Future<void> init(int periodMs) async {
    if (state != EngineState.uninit) {
      throw MiniaudioDartPlatformException("cannot init engine");
    }
  ...
  Future<PlatformSound> loadSound(AudioData data) async {
    if (state == EngineState.uninit) {
      throw MiniaudioDartPlatformException("cannot load sound");
    }
  ...
  void start() {
    if (state != EngineState.init) {
      throw MiniaudioDartPlatformException("cannot start engine");
    }
...
class RecorderMock implements PlatformRecorder {
  ...
  void start() {
    if (state != RecorderState.ready) {
      throw MiniaudioDartPlatformException("Recorder not initialized");
    }
  ...
  void stop() {
    if (state != RecorderState.recording) {
      throw MiniaudioDartPlatformException("Recorder not recording");
    }
    state = RecorderState.ready;
  }
